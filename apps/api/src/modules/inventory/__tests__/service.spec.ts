...
describe('InventoryService.list', () => {
  it('admin can list any store', async () => {
    ...
    await service.list('store-prado-seed', { page: 1, pageSize: 20 }, { userId: 'admin', isAdmin: true });
  ...
  it('staff with valid assignment can list', async () => {
    repo.findUserAssignment.mockResolvedValue({ userId: 'u1', storeId: 'store-prado-seed', role: 'vendedora' });
    repo.list.mockResolvedValue({ items: [], total: 0, page: 1, pageSize: 20 });
    await service.list('store-prado-seed', { page: 1, pageSize: 20 }, { userId: 'u1', isAdmin: false });
...
describe('InventoryService.adjust', () => {
  it('encargada adjusts without checking edit permission', async () => {
    repo.findUserAssignment.mockResolvedValue({ userId: 'u1', storeId: 'store-prado-seed', role: 'encargada' });
    ...
    expect(repo.getEditPermission).not.toHaveBeenCalled();
  ...
  it('vendedora adjusts when toggle is ON', async () => {
    repo.findUserAssignment.mockResolvedValue({ userId: 'u1', storeId: 'store-prado-seed', role: 'vendedora' });
    repo.getEditPermission.mockResolvedValue({ storeId: 'store-prado-seed', isEnabled: true, toggledByUserId: 'admin', toggledAt: new Date().toISOString() });
    repo.findRow.mockResolvedValue(buildRow({ quantity: 5 }));
    repo.upsertQuantity.mockResolvedValue({ previous: 10, next: 5 });

    await service.adjust('store-prado-seed', 'var-1', { quantity: 5 }, { userId: 'u1', isAdmin: false });

    expect(repo.upsertQuantity).toHaveBeenCalled();
  });

  it('vendedora is BLOCKED when toggle is OFF (STOCK_VENDEDORA_EDIT_DISABLED)', async () => {
    repo.findUserAssignment.mockResolvedValue({ userId: 'u1', storeId: 'store-prado-seed', role: 'vendedora' });
    repo.getEditPermission.mockResolvedValue({ storeId: 'store-prado-seed', isEnabled: false, toggledByUserId: null, toggledAt: null });

    await expect(
      service.adjust('store-prado-seed', 'var-1', { quantity: 5 }, { userId: 'u1', isAdmin: false }),
    ).rejects.toMatchObject({ code: 'STOCK_VENDEDORA_EDIT_DISABLED', statusCode: 403 });
    expect(repo.upsertQuantity).not.toHaveBeenCalled();
  });

  it('rejects out-of-scope store with 404', async () => {
    repo.findUserAssignment.mockResolvedValue(null);
    await expect(
  ...
  it('writes a movement row with delta/previous/next/reason', async () => {
    repo.findUserAssignment.mockResolvedValue({ userId: 'u1', storeId: 'store-prado-seed', role: 'encargada' });
...
describe('InventoryService.getByBarcode', () => {
  it('returns the inventory row when found', async () => {
    ...
    const result = await service.getByBarcode('store-prado-seed', 'AAAAAAAAAAAA', { userId: 'u1', isAdmin: false });
...
describe('InventoryService.listMovements', () => {
  it('admin can list movements', async () => {
    ...
    await service.listMovements('store-prado-seed', { page: 1, pageSize: 20 }, { userId: 'admin', isAdmin: true });
  ...
  it('vendedora is blocked (403)', async () => {
    repo.findUserAssignment.mockResolvedValue({ userId: 'u1', storeId: 'store-prado-seed', role: 'vendedora' });
    await expect(
      service.listMovements('store-prado-seed', { page: 1, pageSize: 20 }, { userId: 'u1', isAdmin: false }),
...
describe('InventoryService.togglePermission', () => {
  ...
  it('vendedora cannot toggle (403)', async () => {
    repo.findUserAssignment.mockResolvedValue({ userId: 'u1', storeId: 'store-prado-seed', role: 'vendedora' });
    await expect(
      service.togglePermission('store-prado-seed', { isEnabled: true }, { userId: 'u1', isAdmin: false }),
    ).rejects.toMatchObject({ code: 'STORE_EDIT_PERMISSION_FORBIDDEN', statusCode: 403 });
  });
});
...
describe('InventoryService.getEditPermission', () => {
  it('returns default disabled when row missing', async () => {
    ...
    const result = await service.getEditPermission('store-prado-seed', { userId: 'u1', isAdmin: false });
...
describe('InventoryService — global encargada', () => {
  it('encargada with role=encargada in any store can access any store', async () => {
    ...
    await service.list('store-zsur-seed', { page: 1, pageSize: 20 }, { userId: 'u1', isAdmin: false });
  ...
  it('encargada can list movements of any store', async () => {
    repo.hasAnyEncargadaRole.mockResolvedValue(true);
    repo.listMovements.mockResolvedValue({ items: [], total: 0, page: 1, pageSize: 20 });
    await service.listMovements('store-zsur-seed', { page: 1, pageSize: 20 }, { userId: 'u1', isAdmin: false });
    expect(repo.listMovements).toHaveBeenCalled();
  });
});
...
describe('InventoryService.listGrouped', () => {
  it('returns the grouped result for an authorized user', async () => {
    repo.hasAnyEncargadaRole.mockResolvedValue(true);
    const grouped = {
      items: [{ productId: 'p1', productCode: 'JN001', productName: 'Jean', imagePath: null, totalQuantity: 12, variantsCount: 3 }],
      total: 1, page: 1, pageSize: 20,
    };
    repo.listGroupedByProduct.mockResolvedValue(grouped);

    const result = await service.listGrouped('store-prado-seed', { page: 1, pageSize: 20 }, { userId: 'u1', isAdmin: false });
...
describe('InventoryService.listVariantsForProduct', () => {
  it('returns variants when present', async () => {
    repo.hasAnyEncargadaRole.mockResolvedValue(true);
    repo.listVariantsForProductInStore.mockResolvedValue([buildRow({ size: 'm', color: 'azul', quantity: 5 })]);
    const variants = await service.listVariantsForProduct('store-prado-seed', 'p1', { userId: 'u1', isAdmin: false });
  ...
  it('throws INVENTORY_PRODUCT_NOT_IN_STORE when no variants exist for the product in the store', async () => {
    ...
    await expect(
      service.listVariantsForProduct('store-prado-seed', 'p-missing', { userId: 'u1', isAdmin: false }),
